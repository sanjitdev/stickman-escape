...
// ─── Mock Player ─────────────────────────────────────────────────────────────
function makePlayer(overrides: Partial<MockPlayer> = {}): Player {
...
interface MockPlayer {
  ...
  flipX: boolean;
  arcadeBody: Partial<Phaser.Physics.Arcade.Body>;
  anims: unknown;
  ...
  consumeJump: ReturnType<typeof vi.fn>;
  startCoyoteTimer: ReturnType<typeof vi.fn>;
  startDashCooldown: ReturnType<typeof vi.fn>;
  resetJumpCount: ReturnType<typeof vi.fn>;
  incrementJumpCount: ReturnType<typeof vi.fn>;
  setInvincible: ReturnType<typeof vi.fn>;
...
describe('JumpState', () => {
  it('sets jump velocity on enter', () => {
    const body = { velocity: { x: 0, y: 0 }, setGravityY: vi.fn(), blocked: {} } as unknown as Phaser.Physics.Arcade.Body;
  ...
  it('transitions to fall when velocity turns positive', () => {
    const body = { velocity: { x: 0, y: 10 }, setGravityY: vi.fn(), blocked: {} } as unknown as Phaser.Physics.Arcade.Body;
